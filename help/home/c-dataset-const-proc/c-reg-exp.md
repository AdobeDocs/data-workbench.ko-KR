...
이 부록은 정규 표현식에 대한 포괄적인 소개가 아닙니다. 특히 좋은 추천은 오레일리 출판사입니다 *정규 표현식 관리, 2번째 버전* 제프리 E. F. 프리드먼

## 정규 표현식 용어 {#section-80b0d54f731e448391532ab3eb3c525c}

| 용어 | 정의 |
|---|---|
| 리터럴 | 리터럴은 특정 문자 시퀀스를 찾기 위해 정규 표현식에서 사용하는 문자입니다. 예를 들어에서 제품을 찾으려면 [!DNL shop/products.html]문자열 제품은 문자 그대로 또는 문자열에서 찾을 수 있습니다. |
| 메타문자 | 메타문자는 정규 표현식의 컨텍스트에서 고유한 해석을 가진 특수 문자입니다. 예를 들어 마침표(.)가 는 모든 문자와 일치하는 데 사용되는 메타문자입니다. |
| 이스케이프 시퀀스 | 이스케이프 시퀀스는 단순히 정규 표현식 엔진에 메타문자 중 하나를 리터럴으로 사용하려는 것을 알리는 방법입니다. 이스케이프 시퀀스는 항상 백슬래시 문자(`\`). 상기 정규 표현식 엔진은 상기 메타문자 앞에 백슬래시(메타 문자)를 배치함으로써 상기 이스케이프 처리된 메타문자를 직역적으로 해석한다. 예를 들어 지표 마침표(`.`) 이스케이프 시퀀스를 사용해야 합니다. 그러나 문자열 168.196.0.11에서 점 중 하나를 일치시키려면 백슬래시와 마침표(`\.`). |
...
| 메타문자 | 설명 |
|---|---|
| . (점) | 단일 문자와 일치하는 예: `re:x.z` &quot;xyz&quot; 또는 &quot;xxz&quot;와 일치합니다. |
| * (별) | 하나 이상의 문자와 일치(예: `re:Z*` &quot;ZZZ&quot;와 일치합니다. |
| ? (와일드카드) | 최소 일치를 강제 적용하기 위해 이전 표현식의 0 또는 1과 일치합니다. 예: `xy?z` &quot;xy&quot; 및 &quot;xyz&quot;와 일치합니다. |
...
부정은 주어진 문자를 제외한 모든 항목을 일치시킬 것을 말하는 방법입니다. 부정 메타문자, 원근 또는 캐럿(`^`)를 대괄호 안의 첫 번째 문자로 사용하여 대괄호의 나머지 문자를 제외한 모든 문자를 일치시키겠다고 합니다. 예를 들어, 세미콜론(`;`).
...
| 이 메타문자... | 정규 표현식 프로세서는... |
|---|---|
| 원근 또는 삽입 기호(`^`) | 문자열 시작 부분과 일치합니다. 예: ^`[Tt]`대상 문자열 &quot;시작&quot;과 일치하지만 &quot;이것은 시작&quot;과 일치하지 않습니다. |
| 달러 기호 (`$`) | 문자열 끝에 대해 일치시킵니다. 예, `[Ee]`및$ 은 &quot;This is the end&quot;와 일치하지만 &quot;The end is a special time&quot;과 일치하지 않습니다. |
...
기간(.) 는 대상 문자열의 모든 문자와 일치하는 특수 메타문자입니다. 예를 들어 정규 표현식이 있습니다 `^…$` 정확히 3자 길이의 모든 target 문자열과 일치합니다. 정규 표현식 &quot;...&quot;은(는) 3자 이상을 포함하는 모든 대상 문자열과 일치합니다.
...
패턴 일치는 정규 표현식의 유일한 힘의 일부입니다. 또한 정규 표현식은 대상 문자열의 키 부분을 추출하는 메커니즘을 제공합니다. 이 작업은 왼쪽 및 오른쪽 괄호를 사용하여 수행됩니다. 이러한 추출은 일반적으로 다른 프로세스에 대한 입력으로 사용되며 *%position%*: where position은 괄호가 일치한 개수를 참조하는 정수입니다.
