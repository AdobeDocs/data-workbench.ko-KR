---
description: 센서 오류를 가능한 한 빨리 감지하여 주요 문제 또는 정전을 일으키기 전에 복구하려면 이벤트 로그를 정기적으로 모니터링해야 합니다.
solution: Analytics
title: 관리 이벤트 모니터링
...
센서 오류를 가능한 한 빨리 감지하여 주요 문제 또는 정전을 일으키기 전에 복구하려면 이벤트 로그를 정기적으로 모니터링해야 합니다.

**권장 빈도:** 최소 시간

Windows 이벤트 뷰어 또는 Unix Syslog 파일 및 설치 디렉토리 내의 [!DNL *.sensor-log] 폴더에 기본적으로 있는 파일을 사용하여 이러한 이벤트를 [!DNL Logs] [!DNL Sensor] 모니터링할 수 있습니다. 이러한 파일은 데이터 수집 중 오류가 있음을 나타냅니다. 특히 사용자가 대상에 연결할 [!DNL Sensor] 수 없고 데이터 대기열 [!DNL data workbench server] 을 시작하는 경우입니다.
...
syslog 데몬은 syslog.conf 파일에 지정한 규칙을 기반으로 로그 파일에 오류 메시지를 기록합니다. 심각도에 따라 &quot;LOG_DAEMON&quot; 및 &quot;LOG_NOTICE&quot; 또는 &quot;LOG_ERR&quot; 플래그로 오류가 기록됩니다.

**Unix syslog를 열려면**

Unix syslog는 일반적으로 [!DNL /var/adm/messages] 또는 [!DNL /var/log/messages]에 있습니다.

해당 위치를 찾아 syslog를 엽니다.

## 메시지 형식 이해 {#section-a0899add30fd4b2da58a23b9e3324693}

모든 센서 메시지에는 &quot;Sensor&quot; 문자열이 들어 있으며 표시되는 메시지의 중요도를 반영하도록 번호가 지정됩니다.
...
>[!NOTE]
>
>경고(2xxx)는 현재 사용 중이 아닙니다. 이 번호는 향후 사용을 위해 예약되어 있습니다.

네트워크 관리 도구는 &quot;센서&quot; 소스의 오류에 대해 5-10분마다 메시지를 모니터링하고 개입이 필요할 수 있는 문제에 대해 적절한 직원에게 경고하도록 설정할 수 있습니다. &quot;Sensor Error&quot; 문자열 등 특정 유형의 이벤트 메시지만 시스템을 모니터링하도록 선택할 수 있습니다. 또는 &quot;Sensor Info&quot;, &quot;Sensor Warning&quot; 및 &quot;Sensor Error&quot; 문자열이 앞에 있는 이벤트에 다른 규칙을 적용할 수 있습니다.
...
지원되는 웹 서버 플랫폼에 대한 센서 이벤트 및 제안된 작업을 설명하는 표.
...
<table id="table_F8835AC0AD8F43E2B4494D8D35EBC0FD"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> 센서 정보 1012:Adobe 디스크 큐가 #% 꽉 찼습니다. </td> 
   <td colname="col2"> 이 메시지는 디스크 큐 사용률이 10% 임계값을 넘을 때마다 기록됩니다. 이 비율이 계속 증가하면 큐가 꽉 차서 데이터가 손실되기 전에 조치를 취해야 합니다. 센서가 Insight Server와의 통신을 중지했다는 것이 가장 큰 문제가 되었습니다. Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 센서 정보 1013:센서 구성 변경 </td> 
   <td colname="col2"> 필요한 작업이 없습니다. 센서가 구성 파일 중 하나에서 변경 사항을 감지하여 다시 로드합니다. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> txlogd.conf에서 QueueFile 파일 이름을 확인하고, 올바른 것으로 나타나면 Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> 센서 오류 4021:대기열이 꽉 찼습니다. </td> 
   <td colname="col2"> Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> 센서 오류 5030:포크 오류! </td> 
   <td colname="col2"> Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> 센서 오류 5032:포크 오류! </td> 
   <td colname="col2"> Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> 외부 신호로 프로그램이 종료되었을 가능성이 높습니다. 이 신호의 소스를 확인할 수 없는 경우 Adobe ClientCare에 문의하십시오. </td> 
  </tr> 
  <tr> 
...
| 이벤트 메시지 | 제안된 작업 |
|---|---|
| 센서 오류 3015:VisualSciencesConfig 지시문이 httpd.conf에 없습니다. | 구성 오류입니다. VisualSciencesConfig 지시문은 txlogd.conf의 위치를 나타내는 매개 변수를 사용하여 httpd.conf에 있어야 합니다. |
| 센서 오류 3019:vys-cookie가 vys-log 이전에 호출되지 않았습니다. 지원 센터에 문의하십시오. | Adobe ClientCare에 문의하십시오. |
| 센서 오류 3025:하위 요청 지점을 다시 자신에게 | Adobe ClientCare에 문의하십시오. |

**AOL 서버**

| 이벤트 메시지 | 제안된 작업 |
|---|---|
| 센서 오류 3015:ns/server/[server]/module/[module] 섹션이 AOLServer 구성 파일에 없습니다. | 구성 오류입니다. 오해에 명시된 대로 정합니다. |
...
| 센서 오류 3020:AOLServer 구성 파일의 [섹션] 섹션에 첫 번째 항목으로 VisualSciencesConfig가 없습니다. | 구성 오류입니다. 오해에 명시된 대로 정합니다. |
| 센서 오류 3021:VisualSciencesConfig에 AOLServer 구성 파일의 [섹션] 섹션에 값이 없습니다. | 구성 오류입니다. 오해에 명시된 대로 정합니다. |

**iPlanet 및 Java System 웹 서버**

| 이벤트 메시지 | 제안된 작업 |
|---|---|
| 센서 오류 3011:init 지시문이 필요합니다. Init fn=vys-init config-file=&quot;/mypath/myfile&quot;과 같은 파일 | 구성 오류입니다. iPlanet init 지시문이 없습니다. |
