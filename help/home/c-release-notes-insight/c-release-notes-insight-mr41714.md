---
description: Adobe Analytics는 Adobe Experience Cloud 내에서 프로필 및 대상을 활용하도록 데이터 워크벤치를 강화하는 데이터 피드를 업데이트 및 향상시킵니다. 모든 데이터 워크벤치 사용자는 2014년 4월 21일까지 이 새로운 데이터 피드 형식으로 전환할 준비가 되어 있어야 합니다.
title: 2014년 4월 업데이트
...
# 2014년 4월 업데이트{#updates-for-april-mr}

Adobe Analytics는 Adobe Experience Cloud 내에서 프로필 및 대상을 활용하도록 데이터 워크벤치를 강화하는 데이터 피드를 업데이트 및 향상시킵니다. 모든 데이터 워크벤치 사용자는 2014년 4월 21일까지 이 새로운 데이터 피드 형식으로 전환할 준비가 되어 있어야 합니다.

프로파일과 대상은 Adobe Experience Cloud와 타사 서비스에서 사용할 수 있는 고객에 대한 완전한 실행 가능한 보기를 제공합니다. Analytics에서 새로운 기능에 대한 기반 구축을 시작하기 위해 이번 4월 MR 릴리스의 새로운 Experience Cloud ID 서비스가 다른 개선 사항 및 개선 사항과 함께 데이터 피드에 추가됩니다. 이 새로운 서비스는 Adobe Experience Cloud를 사용하는 모든 고객에게 제공됩니다.

이 새 데이터 피드와 함께 제공된 추가 개선 사항 및 버그 수정은 데이터 워크벤치 구성 준비에 대한 지침과 함께 아래에 나와 있습니다.

## 새 데이터 피드에 대한 개선 사항 및 버그 수정 {#section-0013de07da93411d929a1dfd0417f61b}

* Experience Cloud ID 서비스 추가.
* Analytics 내에서 Target 보고 및 분석을 지원하기 위해 타겟 작업 필드를 추가했습니다.
* 제품 행 유형에 값을 통합하여 보고서 및 분석과의 [!DNL bot_id] 정렬을 개선했습니다.
* 데이터 값에 대한 무결성을 유지하면서 머천다이징 eVar 크기 축소
* 때때로 해시 값이 머천다이징 eVar에 표시되는 문제를 해결했습니다.
* 향상된 안정성과 성능을 지속적으로 제공하기 위해 프로세스 내 내부 개선 사항

## 데이터 워크벤치 구성 업데이트 {#section-381fcd185af6474bb656501c096c451d}

데이터 워크벤치를 변경하면 전환 후 데이터 피드가 중단 없이 통합됩니다. 다음에 예약된 재처리 업데이트 중에 구성의 로그 처리 단계 내의 현재 디코더 그룹이 표시됩니다. 데이터 워크벤치 아키텍처를 관리하는 팀은 새로운 Experience Cloud 방문자 식별자를 노출하고 활용할 수 있도록 추가 작업을 수행할 수 있습니다.

**데이터 워크벤치를 업데이트하는 작업**

1. 현재 디코더 그룹을 찾습니다. 일반적으로 프로필 관리자 내에 있는 데이터 집합\로그 처리\ [!DNL Instructions.cfg] 디코딩에 있습니다.
1. 구성 파일에서 현재 디코더 형식을 마우스 오른쪽 단추로 클릭하고 복사를 선택합니다.
1. 현재 디코더 형식 아래에 새 항목을 마우스 오른쪽 단추로 클릭하고 붙여 넣습니다.
1. 새 디코더 형식을 확장하고 목록 끝에 두 개의 새 필드 위치를 추가합니다. 이러한 위치는 데이터 세트에서 새 필드를 사용할 준비가 되지 않은 경우 비워 두거나 다음과 같이 정의할 수 있습니다.

   * [!DNL x-mcvisid]
   * [!DNL x-tnt-action]

## 디코딩 지침 {#section-ba63132ce9604851ad319b6181011820}
